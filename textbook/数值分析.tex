...
\section{等距节点下得Newton基本差商公式及弗雷瑟图表法}
\subsection{差分}
\begin{definition}
...
$$
	...
	\begin{cases}
		=\sum_{i=0}^nL_i(x)=1 & (\forall i\in\N\cap[0,n]\ (L_i(x)\geq0)) \\
		>1                    & \text{(otherwise)}
	\end{cases}.
$$
...
设间距为$h$，则
$$
	L_i(x)=(-1)^{n-i}\frac{n+1}{\frac{x-x_0}{h}-i}\binom{\frac{x-x_0}{h}}{n+1}\binom{n}{i}\
	(i\in\N,\ i\leq n,\ x\neq x_0+hi).
$$
...
于是有Newton-Cotes求积公式：
$$
	\int_{a}^{b}f(x)\d x\approx(b-a)\sum_{i=0}^{n}f(x_i)c_i^{(n)}.
$$
